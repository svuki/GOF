...
function test(expr, str) {
    if (! expr)
	console.log("TEST FAILED: " + str);
}
function tdarr_eq(td1, td2) {
    if (td1.rows !== td2.rows)
	return false;
    if (td1.cols !== td2.cols)
	return false;
    for (i = 0; i < td1.rows; i++) {
	for (j = 0; j < td1.cols; j++) {
	    if (td1.at(i,j) !== td2.at(i,j))
		return false
	}
    }
    return true;
...
function rle_test() {
    ...
    test(row_to_rle([0, 0, 0]) === "$", "Row_to_rle encodes empty lines");
    test(row_to_rle([]) === "$", "Row to rle encodes empty rows with $");
    test(row_to_rle([1]) === "o$", "Row to rle encodes run length of 1.");
    test(row_to_rle([1,0,1]) === "obo$", "Row to rle encodes multiple runs of length 1.");
    test(row_to_rle([1, 1, 1]) === "3o$", "Row_to_rle encodes multiple length run.");
    test(row_to_rle([1,1,1,0,0,1,1]) === "3o2b2o$", "Row_to_rle encodes multiple multiple length runs.");
    // Test that from_rle and to_rle are inverses
    let tdarr = new TDArray(1, 10, () => Math.random() > 0.5 ? 1 : 0);
    let rule = new Rule("B23/S45");
    let rle = to_rle(tdarr, rule);
    let result = from_rle(rle);
    test(rule.string === result.rule.string, "to_rle->from_rle properly reads rule.");
    test(tdarr_eq(tdarr, result.pattern), "from_rle reproduces encoded tdarr.");
